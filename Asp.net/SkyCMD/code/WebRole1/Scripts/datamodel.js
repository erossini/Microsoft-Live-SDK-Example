(function () {
        ...
        WL.api(
            ...
            function (response) {
                if (!response.error) {
                    ...
                    me.getFile(parent_id, null, true);
                    callback(true);
                }
                else {
                    // failed
        ...
        WL.api(
            ...
            function (response) {
                if (!response.error) {
                    ...
                    me.getFile(destinationId, null, true);
                    me.getFile(file.parent_id, null, true);
    ...
    // FORCEDOWNLOAD: even if the file is cached, redownload
    //
    // if the file is already cached, the callback is called immediately with the cached file
    proto.getFile = function (id, callback, forceDownload) {
        id = id || 'root';
        var file = filesById[id];
        if (!file || (file.isFolder() && file.isLoading) || forceDownload) {
    ...
    function processResponse(id, response, callback) {
        if (!response.error) {
            ...
            if (folder.sortedChildList != children.length) {
                folder.sortedChildList = [];
